...
export default function EmployeeUpdateForm(props) {
  ...
  const [daysOff, setDaysOff] = React.useState(initialValues.daysOff);
  ...
  const resetStateValues = () => {
    ...
    setDaysOff(cleanValues.daysOff ?? []);
    setCurrentDaysOffValue("");
    setErrors({});
  };
  ...
  const [currentDaysOffValue, setCurrentDaysOffValue] = React.useState("");
  const daysOffRef = React.createRef();
  const validations = {
    name: [{ type: "Required" }],
    division: [],
    daysOff: [],
      ...
      <ArrayField
        onChange={async (items) => {
          ...
          if (onChange) {
            ...
            const result = onChange(modelFields);
            values = result?.daysOff ?? values;
          }
          setDaysOff(values);
          setCurrentDaysOffValue("");
        }}
        currentFieldValue={currentDaysOffValue}
        label={"Days off"}
        items={daysOff}
        hasError={errors?.daysOff?.hasError}
        runValidationTasks={async () =>
          await runValidationTasks("daysOff", currentDaysOffValue)
        }
        errorMessage={errors?.daysOff?.errorMessage}
        setFieldValue={setCurrentDaysOffValue}
        inputFieldRef={daysOffRef}
        defaultFieldValue={""}
      >
        <TextField
          label="Days off"
          isRequired={false}
          isReadOnly={false}
          value={currentDaysOffValue}
          onChange={(e) => {
            let { value } = e.target;
            if (errors.daysOff?.hasError) {
              runValidationTasks("daysOff", value);
            }
            setCurrentDaysOffValue(value);
          }}
          onBlur={() => runValidationTasks("daysOff", currentDaysOffValue)}
          errorMessage={errors.daysOff?.errorMessage}
          hasError={errors.daysOff?.hasError}
          ref={daysOffRef}
          labelHidden={true}
          {...getOverrideProps(overrides, "daysOff")}
        ></TextField>
      </ArrayField>
      <Flex
